...
CAR = {
    color: zoom(plt.imread("img/car-{}.png".format(color)), [0.3, 0.3, 1.0])
...
def set_image(obj, data, scale=CAR_SCALE, x=[0.0, 0.0, 0.0, 0.0]):
    ox = x[0]
    oy = x[1]
    angle = x[2]
...
class Scene(object):
    ...
    def __init__(self, ax, snapshot):
        ...
        self.h_future = ax.plot(
            xs[:, 0], xs[:, 1], zorder=50, linestyle="--", color="white", linewidth=1.0
        )[0]
        self.h_past = ax.plot(
            xs[:, 0], xs[:, 1], zorder=40, linestyle="-", color="blue", linewidth=2.0
        )[0]
...
class Visualizer(object):
    ...
    def run(self):
        self.fig, self.ax = plt.subplots(
            1, len(self.snapshot.keys()), sharex=True, sharey=True, figsize=(13, 7)
        )
        self.fig.canvas.mpl_connect("key_press_event", self.key_press)
        self.fig.canvas.mpl_connect("close_event", self.close)
        self.scenes = [
            Scene(ax, self.snapshot.view(key))
        ...
        self.slider = Slider(box, "Time", 0.0, self.T, valinit=0.0)
        self.t = 0.0
...
def test_visualizer():
    ...
    for v in ["A", "B"]:
        traj = world.human[v]
        traj.x[0].set_value([0.0, 0.0, math.pi / 2.0, 0.5])
        for u in traj.u:
            u.set_value([1 if v == "A" else -1, 1])
    world.robots[0].x[0].set_value([-0.13, 0.0, math.pi / 2.0, 0.5])
